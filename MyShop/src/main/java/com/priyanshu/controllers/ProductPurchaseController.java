...
public class ProductPurchaseController {
	...
	public String purchasePage(HttpServletRequest request,HttpSession session ,Model model) {
		...
		if(total>0) {
			...
			for(int i=0;i<items.size();i++) {
				...
				ProductOrder productOrder=new ProductOrder(item.getProductId(), user.getFname(), user.getLname(), LocalDate.now().toString(), item.getModel(), item.getPath(), item.getDesc(), price, user.getUsername());        
	...
	public String myPurchase(Model model,HttpSession session) {
		...
		if(user!=null) {
			ArrayList<ProductOrder> myProductOrders = (ArrayList<ProductOrder>) productRepository.fetchAllDataByUsername(user.getUsername());
